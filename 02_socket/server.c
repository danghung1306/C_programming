...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
     ...
     serv_addr.sin_addr.s_addr = inet_addr("127.0.0.1");  //ip server
     serv_addr.sin_port = htons(portno);      //port number .chuan hoa port number theo chuan network. Big endian/Little endia.
     ...
     while(1){
        ...
        inet_ntop(AF_INET,(struct sockaddr *)&client_addr,s, sizeof s);// Convert address from binary to string, and copy form src to dst
        printf("server: got connection from %s\n", s);
                
        memset(recvbuff, 0, 256);
        //read data from socket 
        fd = read(newsockfd,recvbuff,255);    //block until receive date.
        if (fd < 0) perror("ERROR reading from socket");
        ...
        fd = write(newsockfd,sendbuff,sizeof(sendbuff));
        if (fd < 0) perror("ERROR writing to socket");
